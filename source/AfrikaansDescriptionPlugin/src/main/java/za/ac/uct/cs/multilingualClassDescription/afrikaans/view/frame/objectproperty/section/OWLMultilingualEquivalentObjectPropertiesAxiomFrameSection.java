...
import za.ac.uct.cs.multilingualClassDescription.afrikaans.view.frame.objectproperty.section.row.OWLMultilingualEquivalentObjectPropertiesAxiomFrameSectionRow;
 ...
 *
 */
public class OWLMultilingualEquivalentObjectPropertiesAxiomFrameSection extends AbstractOWLFrameSection<OWLObjectProperty, OWLEquivalentObjectPropertiesAxiom, OWLObjectPropertyExpression> {

    public static final String LABEL = Utils.getDico().getWord(EnumWord.EQUIVALENT_TO_OBJ_PROP_LABEL); //"Equivalent To"

    private Set<OWLEquivalentObjectPropertiesAxiom> added = new HashSet<>();


    public OWLMultilingualEquivalentObjectPropertiesAxiomFrameSection(OWLEditorKit editorKit,
                                                                      OWLFrame<? extends OWLObjectProperty> frame) {
        super(editorKit, LABEL, Utils.getDico().getWord(EnumWord.EQUIVALENT_TO_ROW_OBJ_PROP_LABEL) , frame); //"Equivalent object property"
    }
    ...
    protected void refill(OWLOntology ontology) {
        ...
        for (OWLEquivalentObjectPropertiesAxiom ax : ontology.getEquivalentObjectPropertiesAxioms(getRootObject())) {
            addRow(new OWLMultilingualEquivalentObjectPropertiesAxiomFrameSectionRow(getOWLEditorKit(),
                                                                         this,
                                                                         ontology,
                                                                         getRootObject(),
                                                                         ax));
                ...
                () -> {
                    ...
                    if (!equivalentObjectProperties.getEntitiesMinus(getRootObject()).isEmpty()) {
                        ...
                        if (!added.contains(ax)) {
                            addInferredRowIfNontrivial(new OWLMultilingualEquivalentObjectPropertiesAxiomFrameSectionRow(getOWLEditorKit(),
                                                                                         OWLMultilingualEquivalentObjectPropertiesAxiomFrameSection.this,
                                                                                         null,
                                                                                         getRootObject(),
                                                                                         ax));
                        }
